...
<body class="bg-gradient-to-br from-blue-50 to-gray-100">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="export-btn-group">
                            <button onclick="exportToPDF()" 
                                    class="export-btn bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg text-sm">
                                <i class="fas fa-file-pdf mr-2"></i>Export PDF
                            </button>
                            <button onclick="exportToExcel()" 
                                    class="export-btn bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg text-sm">
    ...
    <script>
        ...
        function exportToPDF() {
            ...
            const title = "Historique des Pesages - Port Autonome";
            ...
            doc.text(`Date d'export : ${dateExport}`, 14, 22);
            ...
            const typeCamion = document.getElementById('type_camion').value;
            const etat = document.getElementById('etat').value;
            
            if (search) filters.push(`Immatriculation: ${search}`);
            if (dateDebut) filters.push(`Date début: ${dateDebut}`);
            if (dateFin) filters.push(`Date fin: ${dateFin}`);
            if (typeCamion) {
                const typeCamionText = document.getElementById('type_camion').options[document.getElementById('type_camion').selectedIndex].text;
                filters.push(`Type camion: ${typeCamionText}`);
            }
            if (etat) filters.push(`État: ${etat}`);
            ...
            // Télécharger le PDF
            doc.save(`historique_pesages_${dateExport.replace(/\//g, '-')}.pdf`);
        }
        ...
        function exportToExcel() {
            ...
            // Supprimer la colonne Action (dernière colonne)
            const range = XLSX.utils.decode_range(ws['!ref']);
            for (let R = range.s.r; R <= range.e.r; ++R) {
                delete ws[XLSX.utils.encode_cell({c: 5, r: R})]; // Supprimer la colonne 5 (Action)
            }
            // Ajuster la plage de données
            range.e.c = 4; // Réduire à 5 colonnes (0-4)
            ws['!ref'] = XLSX.utils.encode_range(range);
            
            // Ajouter un en-tête personnalisé
            const title = "Historique des Pesages - Port Autonome";
            const titleRow = [title];
            const exportDateRow = [`Date d'export : ${dateExport}`];
            const emptyRow = [''];
            ...
            const filterRow = [filterInfo + filters.join(' | ')];
            
            // Décaler les données existantes vers le bas
            const data = XLSX.utils.sheet_to_json(ws, {header: 1});
            data.unshift(filterRow);
            data.unshift(emptyRow);
            data.unshift(exportDateRow);
            data.unshift(titleRow);
            
            // Créer une nouvelle feuille avec les données mises à jour
            const newWs = XLSX.utils.aoa_to_sheet(data);
            
            // Définir les largeurs de colonnes
            newWs['!cols'] = [
                { wch: 20 }, // Date pesage
                { wch: 25 }, // Immatriculation
                { wch: 15 }, // État
                { wch: 20 }, // Poids total
                { wch: 15 }  // Surcharge
            ];
            ...
            XLSX.utils.book_append_sheet(wb, newWs, "Historique Pesages");
            
            // Générer et télécharger le fichier
            XLSX.writeFile(wb, `historique_pesages_${dateExport.replace(/\//g, '-')}.xlsx`);
        }
        
        // Fonction pour imprimer les détails (conservée pour compatibilité)
        function printDetails() {
            if (!<?php echo $selected_camion_id ? 'true' : 'false'; ?>) return;
            
            const printWindow = window.open('', '_blank');
            const camion = <?php echo json_encode($selected_camion); ?>;
            const pesage = <?php echo json_encode($pesage_details); ?>;
            const marchandises = <?php echo json_encode($marchandises); ?>;
            
            const poidsTotal = (pesage.ptav || 0) + (pesage.poids_total_marchandises || 0);
        }
