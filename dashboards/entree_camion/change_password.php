    ...
    if (empty($ancien_mdp) || empty($nouveau_mdp) || empty($confirmer_mdp)) {
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($ancien_mdp, $db_user['mdp'])) {
                // Vérifier que le nouveau mot de passe n'est pas identique à l'ancien (double vérification)
                if (password_verify($nouveau_mdp, $db_user['mdp'])) {
                    $error = "Le nouveau mot de passe ne peut pas être identique à l'ancien";
                } else {
...
<body class="bg-gray-50">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" 
                                       ...
                                       name="nouveau_mdp" 
                                       class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-blue-500 pl-10"
                                       placeholder="Minimum 6 caractères, différent de l'ancien"
                                       required>
                                <div class="absolute left-3 top-1/2 transform -translate-y-1/2">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-2">
                            ...
                            <p class="text-xs text-gray-600 mt-1">
                                ...
                                <span class="text-red-500 font-medium">Le nouveau mot de passe doit être différent de l'ancien.</span>
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function checkSamePassword() {
            const oldPassword = document.getElementById('ancien_mdp').value;
            const newPassword = document.getElementById('nouveau_mdp').value;
            const submitBtn = document.getElementById('submitBtn');
            ...
            if (oldPassword === newPassword) {
                ...
                if (!document.getElementById('samePasswordError')) {
                    const errorDiv = document.createElement('div');
                    errorDiv.id = 'samePasswordError';
                    errorDiv.className = 'text-xs text-red-600 mt-1';
                    errorDiv.innerHTML = '<i class="fas fa-exclamation-circle mr-1"></i>Le nouveau mot de passe ne peut pas être identique à l\'ancien';
                    
                    const newPasswordDiv = document.getElementById('nouveau_mdp').parentElement;
                    newPasswordDiv.parentElement.appendChild(errorDiv);
                }
                submitBtn.disabled = true;
            } else {
                // Supprimer le message d'erreur s'il existe
                const errorDiv = document.getElementById('samePasswordError');
                if (errorDiv) {
                    errorDiv.remove();
                }
                // Ne réactiver le bouton que si les mots de passe correspondent
                checkPasswordMatch();
            }
        }
        ...
        document.getElementById('ancien_mdp').addEventListener('input', function() {
            checkSamePassword();
        });
