...
if (isset($_GET['select']) && is_numeric($_GET['select'])) {
    ...
    try {
        // Récupérer les informations du bateau avec les noms des types et ports
        $stmt = $conn->prepare("
            SELECT 
                be.*, 
        ...
        if ($result->num_rows > 0) {
            ...
            error_log("Données du bateau récupérées: " . print_r($selected_bateau, true));
            
            // Récupérer TOUJOURS les marchandises associées pour les afficher en mode édition
            $stmt = $conn->prepare("
                SELECT mbe.*, tm.nom as type_marchandise, tm.id as id_type_marchandise
...
// Traitement du formulaire d'entrée de bateau
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'enregistrer') {
        try {
            ...
            // CORRECTION : Assurer que l'état a toujours une valeur
            $etat = trim($_POST['etat'] ?? 'vide');
            if ($etat === '') {
                $etat = 'vide';
            }
            $note = $_POST['note'] ?? '';
            $date_entree = date('Y-m-d H:i:s'); // Date automatique actuelle
            ...
            error_log("Valeur de l'état après récupération: " . $etat);
            
            if ($selected_bateau_id && isset($_POST['id_entree']) && !empty($_POST['id_entree'])) {
                // Mise à jour de l'entrée existante
                $stmt = $conn->prepare("
                    UPDATE bateau_entrant 
                    ...
                    WHERE id = ?
                ");
                error_log("Exécution de la requête de mise à jour avec l'état: " . $etat);
                $stmt->bind_param("issssssdddissi", 
                    $id_type_bateau, $nom_navire, $immatriculation,
                    $nom_capitaine, $prenom_capitaine, $tel_capitaine,
                    $agence, $hauteur, $longueur, $largeur,
                    $id_port, $etat, $note, $selected_bateau_id
                );
                if (!$stmt->execute()) {
                    error_log("Erreur lors de la mise à jour: " . $stmt->error);
                    throw new Exception("Erreur lors de la mise à jour: " . $stmt->error);
                }
                $idEntree = $selected_bateau_id;
                error_log("Mise à jour réussie pour le bateau ID: " . $selected_bateau_id);
                
                // Supprimer les anciennes marchandises si le bateau passe à vide
                if ($etat == 'vide') {
                    $stmt = $conn->prepare("DELETE FROM marchandise_bateau_entrant WHERE id_bateau_entrant = ?");
                    $stmt->bind_param("i", $selected_bateau_id);
                    $stmt->execute();
                    error_log("Marchandises supprimées car bateau vide");
                }
            } else {
                // Nouvelle entrée de bateau
                $agent_name = $user['prenom'] . ' ' . $user['nom'];
                $stmt = $conn->prepare("
                    INSERT INTO bateau_entrant 
                    (id_type_bateau, nom_navire, immatriculation, nom_capitaine, 
                     prenom_capitaine, tel_capitaine, agence, hauteur, longueur, 
                     largeur, id_port, etat, note, date_entree, agent_enregistrement)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                $stmt->bind_param("issssssdddissss", 
                    ...
                    $nom_capitaine, $prenom_capitaine, $tel_capitaine,
                    $agence, $hauteur, $longueur, $largeur,
                    $id_port, $etat, $note, $date_entree, $agent_name
                );
                ...
                $idEntree = $stmt->insert_id;
                error_log("Nouveau bateau inséré avec l'ID: " . $idEntree . ", État: " . $etat);
            }
            ...
            if ($etat == 'chargé' && isset($_POST['marchandises']) && is_array($_POST['marchandises'])) {
                ...
                foreach ($_POST['marchandises'] as $marchandise) {
                    if (!empty($marchandise['type']) && $marchandise['type'] > 0) {
                        ...
                        $stmt = $conn->prepare("
                            INSERT INTO marchandise_bateau_entrant 
                            (id_bateau_entrant, id_type_marchandise, poids, note)
                            VALUES (?, ?, ?, ?)
                        ");
                        $stmt->bind_param("iids", $idEntree, $type_id, $poids, $note_marchandise);
                        $stmt->execute();
                    }
                }
                error_log("Marchandises enregistrées pour le bateau ID: " . $idEntree);
            }
            
            $success = "Entrée de bateau enregistrée avec succès!";
            
            // Rediriger en mode view après enregistrement
            if ($idEntree) {
                header("Location: enregistrement.php?select=" . $idEntree . "&mode=view");
                exit();
            }
...
// Récupérer les types de marchandises, types de bateaux et ports
$types_marchandises = [];
$types_bateaux = [];
$ports = [];
...
try {
    ...
    // Récupérer les enregistrements récents avec les noms des ports
    $sql = "SELECT be.*, tb.nom as type_bateau, p.nom as port_nom
            FROM bateau_entrant be 
            LEFT JOIN type_bateau tb ON be.id_type_bateau = tb.id
            LEFT JOIN port p ON be.id_port = p.id";
    
    if (!empty($search)) {
        $sql .= " WHERE be.nom_navire LIKE ? OR be.immatriculation LIKE ?";
        ...
        $sql .= " ORDER BY be.date_entree DESC LIMIT 10";
...
<head>
    ...
    <title>Enregistrement des Bateaux</title>
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            <h2 class="text-base font-bold text-gray-800">
                                ...
                                <?php echo ($selected_bateau && $mode == 'view') ? 'Consultation Bateau' : ($selected_bateau ? 'Modification Bateau' : 'Nouveau Bateau'); ?>
                            </h2>
                ...
                <div class="p-3">
                    <form method="POST" id="bateauForm">
                        ...
                        <?php if ($selected_bateau): ?>
                            ...
                            <input type="hidden" name="id_entree" value="<?php echo $selected_bateau['id']; ?>">
                        <?php endif; ?>
                        ...
                        <div id="step1" class="step <?php echo ($mode != 'view') ? 'active' : ''; ?>">
                            ...
                            <div class="compact-section">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    ...
                                    <div>
                                        <label class="compact-label block text-gray-700 font-bold" for="id_port">
                                            Port de provenance *
                                        </label>
                                        <select id="id_port" name="id_port"
                                            ...
                                            <?php foreach ($ports as $port): ?>
                                                <option value="<?php echo $port['id']; ?>"
                                                    <?php echo isset($selected_bateau['id_port']) && $selected_bateau['id_port'] == $port['id'] ? 'selected' : ''; ?>>
...
<?php endif; ?>
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="p-3 border-b">
                    <div class="flex justify-between items-center">
                        <h2 class="text-base font-bold text-gray-800">
                            <i class="fas fa-list mr-1"></i>Enregistrements Récents
                        </h2>
                        <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-0.5 rounded">
                                ...
                                <?php if (!empty($recent_bateaux)): ?>
                                    <?php foreach ($recent_bateaux as $bateau): ?>
                                        <tr class="border-b recent-bateau-row <?php echo $selected_bateau_id == $bateau['id'] ? 'bg-blue-50' : ''; ?>">
                                            ...
                                            <td class="py-2 px-2">
                                                <?php echo safe_html($bateau['port_nom'] ?? '-'); ?>
                                            </td>
                                            <td class="py-2 px-2">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" class="py-6 text-center text-gray-500 text-xs">
                                            ...
                                            <?php else: ?>
                                                <p class="text-xs mt-0.5">Enregistrez un nouveau bateau</p>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endif; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form && !isViewMode) {
                form.addEventListener('submit', function(e) {
                    ...
                    // Si le bateau est chargé, vérifier qu'il y a au moins une marchandise
                    if (etat === 'chargé') {
                        ...
                        // Vérifier que toutes les marchandises ont un poids
