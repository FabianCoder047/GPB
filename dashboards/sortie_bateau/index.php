...
try {
    ...
    $query = "SELECT COUNT(*) as total 
              FROM bateau_sortant 
              WHERE YEARWEEK(date_sortie, 1) = YEARWEEK(CURDATE(), 1)";
    ...
    $query = "SELECT COUNT(*) as total 
              FROM bateau_sortant 
              WHERE YEAR(date_sortie) = YEAR(CURDATE()) 
    ...
    $query = "SELECT COUNT(*) as total, 
                     COUNT(DISTINCT DATE(date_sortie)) as jours
              FROM bateau_sortant 
              WHERE date_sortie >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
    $result = $conn->query($query);
    if ($row = $result->fetch_assoc()) {
    ...
    $query = "SELECT tb.nom, COUNT(*) as count
              ...
              JOIN type_bateau tb ON bs.id_type_bateau = tb.id
              WHERE YEAR(bs.date_sortie) = YEAR(CURDATE()) 
              AND MONTH(bs.date_sortie) = MONTH(CURDATE())
              GROUP BY tb.nom
              ORDER BY count DESC
              LIMIT 5";
    $result = $conn->query($query);
    ...
    $query = "SELECT DATE(date_sortie) as date, 
                     COUNT(*) as total,
                     SUM(CASE WHEN etat = 'chargé' THEN 1 ELSE 0 END) as charges,
                     SUM(CASE WHEN etat = 'vide' THEN 1 ELSE 0 END) as vides
              FROM bateau_sortant
              WHERE date_sortie >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
...
<head>
    ...
    <title>Tableau de bord - Enregistreur Sortie Bateaux</title>
...
<body class="bg-gradient-to-br from-green-50 to-gray-100 min-h-screen">
    ...
    <div class="container mx-auto p-4">
        ...
        <div class="dashboard-grid mb-8">
                ...
                <div class="flex justify-between items-start">
                    <div>
                        <div class="stat-card-icon" style="background-color: rgba(16, 185, 129, 0.1);">
                            <i class="fas fa-ship text-xl" style="color: #10b981;"></i>
                        </div>
            ...
            <div class="dashboard-card stat-card p-6" style="border-top-color: #0ea5e9;">
                <div class="flex justify-between items-start">
                    <div>
                        <div class="stat-card-icon" style="background-color: rgba(14, 165, 233, 0.1);">
                            <i class="fas fa-calendar-alt text-xl" style="color: #0ea5e9;"></i>
                        </div>
                        <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wide">
                            Ce mois-ci
                        </h3>
                    ...
                    <div class="text-right">
                        <span class="text-xs font-medium px-2.5 py-0.5 rounded-full bg-sky-100 text-sky-800">
                            <?php echo date('F'); ?>
                        </span>
                    </div>
                </div>
        ...
        <div class="dashboard-card p-6 mb-8">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-bold text-gray-800">
                    <i class="fas fa-history mr-2"></i>Sorties récentes
                </h2>
                <a href="historiques.php" class="text-sm text-green-600 hover:text-green-800 font-medium">
            ...
            <div class="overflow-y-auto" style="max-height: 400px;">
                <?php if (!empty($stats['activite_recente'])): ?>
                    <div class="space-y-0">
                        <?php foreach ($stats['activite_recente'] as $bateau): ?>
                            <div class="activity-item px-2 py-3 rounded-lg hover:bg-green-50">
                                <div class="flex items-center">
                ...
                <?php else: ?>
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500">Aucune sortie récente</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
            ...
            function updateTime() {
                ...
                document.querySelector('.text-green-700').textContent = `${dateString}`;
            }
    </script>
    <script>
