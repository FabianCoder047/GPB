...
try {
    ...
    $query = "
        SELECT bs.*, tb.nom as type_bateau, p.nom as destination_port_nom,
               (SELECT COUNT(*) FROM marchandise_bateau_sortant WHERE id_bateau_sortant = bs.id) as nb_marchandises
    ...
    if (!empty($search)) {
        $query .= " AND (bs.nom_navire LIKE ? OR bs.immatriculation LIKE ? OR bs.nom_capitaine LIKE ? OR bs.prenom_capitaine LIKE ?)";
        $params[] = '%' . $search . '%';
        $params[] = '%' . $search . '%';
        $params[] = '%' . $search . '%';
        $params[] = '%' . $search . '%';
    ...
    if (!empty($type_bateau) && is_numeric($type_bateau)) {
        $query .= " AND bs.id_type_bateau = ?";
        $params[] = $type_bateau;
        $types .= 'i';
    }
    
    // Ajouter l'ordre et la limite
    $query .= " ORDER BY bs.date_sortie DESC";
...
<head>
    ...
    <title>Historique des Bateaux - Sorties</title>
        ...
        <div class="bg-white shadow rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex flex-col md:flex-row md:items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-gray-800">
                            <i class="fas fa-list mr-2"></i>Liste des Sorties de Bateaux
                        </h2>
                        <p class="text-sm text-gray-600 mt-1">
                            <?php echo $total_bateaux; ?> sortie(s) de bateau(x) trouvée(s) 
                        ...
                        <tr class="hover:bg-green-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">
                                    ...
                                    <?php echo safe_html($bateau['destination_port_nom'] ?? 'Non spécifié'); ?>
                                </div>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    ...
                                    <p class="text-lg font-medium text-gray-500">Aucune sortie de bateau trouvée</p>
            ...
            <div id="cardView" class="hidden p-6 card-grid">
                ...
                <div class="bateau-card bg-white p-4 hover:shadow-lg">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <div class="flex items-center">
                                <div class="h-8 w-8 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center mr-2">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex items-center text-gray-600">
                            ...
                            <span><?php echo safe_html($bateau['destination_port_nom'] ?? 'Non spécifié'); ?></span>
                        </div>
                        <div class="flex items-center text-gray-600">
                        ...
                        <div class="flex items-center text-gray-600">
                            ...
                            <span><?php echo date('d/m/Y', strtotime($bateau['date_sortie'] ?? '')); ?></span>
                        </div>
                    </div>
                    
                    <div class="mt-4 pt-3 border-t">
                        <button onclick="openModal(<?php echo $bateau['id']; ?>)" 
                                class="w-full inline-flex items-center justify-center px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 rounded-lg text-sm font-medium transition duration-200">
    ...
    <script>
            ...
            try {
                                ...
                                <div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <div class="space-y-2">
                                            ...
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Port de destination:</span>
                                                <span class="font-medium">${data.destination_port_nom || 'Non spécifié'}</span>
                                            </div>
                                            <div class="flex justify-between">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <div class="space-y-2">
                                            <div class="flex justify-between">
                                                ...
                                                <span class="font-medium">${new Date(data.date_sortie).toLocaleDateString('fr-FR')}</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Heure de sortie:</span>
                                                <span class="font-medium">${new Date(data.date_sortie).toLocaleTimeString('fr-FR')}</span>
                                            </div>
                                            <div class="flex justify-between">
                ...
                document.getElementById('modalContent').innerHTML = `
                    <div class="p-8 text-center">
                        ...
                        <button onclick="closeModal()" 
                                class="mt-4 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg">
                            Fermer
                        </button>
                    </div>
                `;
        ...
        function exportToExcel() {
            ...
            if (navigator.msSaveBlob) {
                navigator.msSaveBlob(blob, "historiques_sorties.csv");
            } else {
                link.href = URL.createObjectURL(blob);
                link.download = "historiques_sorties_" + new Date().toISOString().split('T')[0] + ".csv";
                link.click();
            }
        }
